...
# AS WE CHANGE THE QAOA ANGLE THETA. WE KEEP THE QAOA ANGLE GAMMA FIXED AT ITS OPTIMUM VALUE.
# **********************************
...
def experiment(data):
    ...
    # Optimal angles
    _, γ = optimization(N, E, cost_function=objective_function)
    θ_array = np.linspace(0.001, math.pi, 31)
...
def main_job(
    ...
    for ptname in problem_types:
        for graph_type, graph_arguments in graphs:
            outputfile = f"expts/exp_VStheta-{ptname}_nq{nmin}-{nmax}_nsamples{sample_size}_{graph_type}{graph_arguments}_{objective_function}.pkl"
...
if __name__ == "__main__":
    # main_job(workers=12, nmin=4, nmax=18)
    # main_job(mpi=True, nmin=4, nmax=24)
    # main_job(mpi=True, nmin=4, nmax=20)
